...
class Commands:
    ...
    def __init__(self):
        ...
        self.comm_dict = {}
        self.reg_file = 'reg_file.p'
        # Load from file if exists
        if os.stat(self.reg_file).st_size > 0:
            self.reg_dict = pickle.load(open(self.reg_file, 'rb'))
        else:
            self.reg_dict = {}
    ...
    def create(self, name, re):
        ...
        """
        if len((str(name)+str(re)).split(" ")) > 2:
            return "Please give me exactly two arguments - name and reg expression"
        self.reg_dict[str(name)] = str(re)
        pickle.dump(self.reg_dict, open(self.reg_file, 'wb'))
    ...
    def ls(self):
        ...
        """
	if os.stat(self.reg_file).st_size > 0:
            self.reg_dict.update(pickle.load(open(self.reg_file, 'rb')))
        string_reg = "Available reg expressions: \n"
        for key in self.reg_dict:
            string_reg += str(key) + ": " + str(self.reg_dict[key]) + "\n"
        return string_reg
